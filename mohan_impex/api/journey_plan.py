...
def journey_plan_form():
    ...
    try:
        ...
        journey_doc = frappe.get_doc("Journey Plan", journey_name).as_dict()
        ...
        def get_label(doctype, name_value, label_field):
            if not name_value:
                return ""
            value = frappe.db.get_value(doctype, {"name": name_value}, label_field)
            return value or name_value

        ordered_trips = []

        for trip in trips:
            primary_customer = csv_to_list(trip.get("primary_customer"))
            secondary_customer = csv_to_list(trip.get("secondary_customer"))

            from_state = trip.get("travel_state_from")
            from_dist = trip.get("travel_from_district")
            from_city = trip.get("travel_from_city")

            to_state = trip.get("travel_to_state")
            to_dist = trip.get("travel_to_district")
            to_city = trip.get("travel_to_city")

            from_state_name = get_label("State", from_state, "state")
            from_dist_name = get_label("District", from_dist, "district")
            from_city_name = get_label("City", from_city, "city")

            to_state_name = get_label("State", to_state, "state")
            to_dist_name = get_label("District", to_dist, "district")
            to_city_name = get_label("City", to_city, "city")
            ...
            ordered_trip = {
                ...
                "travel_state_from": from_state,
                "travel_state_from_name": from_state_name,
                "travel_from_district": from_dist,
                "travel_from_district_name": from_dist_name,
                "travel_from_city": from_city,
                "travel_from_city_name": from_city_name,

                "travel_to_state": to_state,
                "travel_to_state_name": to_state_name,
                "travel_to_district": to_dist,
                "travel_to_district_name": to_dist_name,
                "travel_to_city": to_city,
                "travel_to_city_name": to_city_name,
            }

            ordered_trips.append(ordered_trip)

        journey_doc["trips"] = ordered_trips
