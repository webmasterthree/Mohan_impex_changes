...
#     emp_filters = {"status": "Active","default_shift":"Night Shift"}
#     if employee:
...
def daily_attendance_log(from_date=None, to_date=None, employee=None):
    ...
    # 🔹 STEP 1: Employees (Night Shift only)
    emp_filters = {
        "status": "Active",
        "default_shift": "Night Shift"
    }
    ...
    shift_data = frappe.db.get_all(
        "Shift Type",
        filters={"name": "Night Shift"},
        fields=["name", "start_time", "end_time"]
    )

    shift_info = shift_data[0] if shift_data else {}
    start_td = shift_info.get("start_time")
    end_td = shift_info.get("end_time")
    ...
    holiday_lists = list(set([
        emp.get("holiday_list")
        for emp in employees if emp.get("holiday_list")
    ]))
    ...
    for emp_id in sorted(emp_map.keys()):
        ...
        for dt in all_dates:
            ...
            # 🔹 Same Day Logs
            for log in logs:
                log_key = (emp_id, log["time"])

                if log["log_type"] == "IN" and not check_in:
                    check_in = log["time"]

                elif log["log_type"] == "OUT" and log_key not in used_checkout:
                    check_out = log["time"]
                    used_checkout.add(log_key)

            # 🔥 Next Day Checkout (only if needed)
            if not check_out:
                next_day = frappe.utils.add_days(dt, 1)
                next_day_logs = checkin_map.get(emp_id, {}).get(next_day, [])

                for log in next_day_logs:
                    log_key = (emp_id, log["time"])

                    if log["log_type"] == "OUT" and log_key not in used_checkout:
                        check_out = log["time"]
                        used_checkout.add(log_key)
                        break
            ...
            # 🔥 Night Shift Cross Midnight Fix
            if start_td and end_td and end_td < start_td:
