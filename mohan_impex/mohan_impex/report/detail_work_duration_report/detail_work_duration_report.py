...
def convert_to_hours_minutes(value):
    """decimal hours -> 'X hours, Y min' ; '-' for zero/None"""
    if isinstance(value, (int, float)):
        hours = abs(int(value))
        minutes = abs(round((value - int(value)) * 60))
        return f"{hours} hours, {minutes} min" if (hours or minutes) else "-"
...
def transform_to_required_format(attendance_data, date_headers):
    """Row-wise per employee with sub-rows for fields."""
    final_data = []
    for emp_id, record in attendance_data.items():
        emp_details = [
            record.get("Employee ID"), record.get("Employee Name"), record.get("Department"),
            record.get("Branch"), record.get("Reports To"), record.get("Company"),
            record.get("Shift"), record.get("Shift Start"), record.get("Shift End")
        ]
        final_data.append(emp_details + [""] * len(date_headers))
...
def execute(filters=None):
    ...
    selected_branch = filters.get("branch")
    selected_reports_to = filters.get("reports_to")
    selected_custom_reports_two = filters.get("custom_reports_two")   # <-- now supported
    selected_company = filters.get("company")
    selected_shift = filters.get("shift")
    ...
    while current_date <= end_date:
        ...
        for record in response.get("data", []):
            # Apply ALL filters safely (compare against DocNames)
            if selected_employee and record.get("employee") != selected_employee:
                continue
            if selected_department and record.get("department") != selected_department:
                continue
            if selected_branch and record.get("branch") != selected_branch:
                continue
            if selected_reports_to and record.get("reports_to") != selected_reports_to:
                continue
            if selected_custom_reports_two and record.get("custom_reports_two") != selected_custom_reports_two:
                continue
            if selected_company and record.get("company") != selected_company:
                continue
            if selected_shift and record.get("shift") != selected_shift:
                continue

            emp_id = record.get("employee")
            if emp_id not in attendance_data:
                attendance_data[emp_id] = {
                    "Employee ID": record.get("employee"),
                    "Employee Name": record.get("employee_name"),
                    "Department": record.get("department"),
                    "Branch": record.get("branch"),
                    "Reports To": record.get("reports_to"),
                    "Company": record.get("company"),
                    "Shift": record.get("shift"),
                    "Shift Start": format_time(record.get("shift_start")) if record.get("shift_start") else "",
                    "Shift End": format_time(record.get("shift_end")) if record.get("shift_end") else "",
                    "Attendance": {}
                }
            ...
            attendance_data[emp_id]["Attendance"][formatted_date].update({
                "Check-In": format_time(in_time) if in_time else "-",
                "Check-Out": format_time(out_time) if out_time else "-",
                "Status": record.get("status", "Absent"),
                "Total Hours": convert_to_hours_minutes(
                    time_diff_in_hours(in_time, out_time)
                ) if in_time and out_time else "-",
                "Over Time": convert_to_hours_minutes(
                    time_diff_in_hours(out_time, shift_end)
                ) if out_time and shift_end and out_time > shift_end else "-",
                "Late By": convert_to_hours_minutes(
                    time_diff_in_hours(shift_start, in_time)
                ) if shift_start and in_time and in_time > shift_start else "-",
                "Early By": convert_to_hours_minutes(
                    time_diff_in_hours(out_time, shift_end)
                ) if out_time and shift_end and out_time < shift_end else "-"
            })
    ...
    columns = [
        "Employee ID", "Employee Name", "Department", "Branch", "Reports To",
        "Company", "Shift", "Shift Start", "Shift End"
    ] + date_headers
