...
def get_delivery_note_from_sales_invoice(name):
    """Return Delivery Note and related Purchase Invoices (transporter / misc / labour) for a Sales Invoice."""
    if not name:
        return None
    ...
    group_wise_columns = frappe._dict(
        {
            "invoice": [
                ...
                "base_rate",
                "buying_rate",
                "total_base_amount",  # NEW: items-only selling total (invoice header)
                "base_amount",        # Selling Amount (items + expenses for header)
                "buying_amount",
                "gross_profit",
...
def get_data_when_grouped_by_invoice(columns, gross_profit_data, filters, group_wise_columns, data):
    ...
    for src in gross_profit_data.si_list:
        # totals from item rows (items-only)
        if src.indent == 1:
            total_base_amount += src.base_amount or 0.0
            total_buying_amount += src.buying_amount or 0.0

        # totals of extra expenses from invoice header rows
        if src.indent == 0:
            total_transporter_amount += src.get("transporter_amount") or 0.0
            total_misc_amount += src.get("misc_amount") or 0.0
            total_labour_amount += src.get("labour_amount") or 0.0
        ...
        # hidden PI ids (for JS formatter)
        row["transporter_pi"] = src.get("transporter_pi")
        row["misc_pi"] = src.get("misc_pi")
        row["labour_pi"] = src.get("labour_pi")

        data.append(row)

    # total Selling Amount = items + all three expenses
    total_extra_expense = (total_transporter_amount or 0) + (total_misc_amount or 0) + (total_labour_amount or 0)
    net_selling_total = total_base_amount + total_extra_expense

    total_gross_profit = net_selling_total - total_buying_amount
    data.append(
        frappe._dict(
            {
                "sales_invoice": "Total",
                ...
                "valuation_rate": None,
                # NEW column value in total row
                "items_selling_amount": total_base_amount,
                # existing selling amount (items + expenses)
                "selling_amount": net_selling_total,
                "buying_amount": total_buying_amount,
                "gross_profit": total_gross_profit,
                "gross_profit_%": flt(
                    (total_gross_profit / net_selling_total) * 100.0,
                    cint(frappe.db.get_default("currency_precision")) or 3,
                )
                if net_selling_total
                else 0,
                "transporter_amount": total_transporter_amount,
                "misc_amount": total_misc_amount,
                "labour_amount": total_labour_amount,
            }
        )
    )
    ...
    column_map = frappe._dict(
        {
            ...
            # NEW: items-only invoice total column (header rows)
            "total_base_amount": {
                "label": _("Base Selling Amount"),
                "fieldname": "items_selling_amount",
                "fieldtype": "Currency",
                "options": "currency",
                "width": 140,
            },
            ...
            # Only amount columns for expenses (no PI ids)
            "transporter_amount": {
                "label": _("Transporter Expense"),
    ...
    return frappe._dict(
        {
            ...
            # NEW mapping
            "total_base_amount": "items_selling_amount",
...
class GrossProfitGenerator:
    ...
    def process(self):
        ...
        for row in reversed(self.si_list):
            ...
            # Attach PI info for invoice header rows
            if grouped_by_invoice and row.indent == 0.0:
                invoice_name = row.invoice_or_item
                if invoice_name not in pi_cache:
                    pi_cache[invoice_name] = get_delivery_note_from_sales_invoice(invoice_name)

                info = pi_cache.get(invoice_name) or {}
                t = info.get("transporter") or {}
                m = info.get("misc_expense") or {}
                l = info.get("labour_payment") or {}

                row.transporter_pi = t.get("name")
                row.transporter_amount = t.get("rounded_total") or 0
                row.misc_pi = m.get("name")
                row.misc_amount = m.get("rounded_total") or 0
                row.labour_pi = l.get("name")
                row.labour_amount = l.get("rounded_total") or 0

            if self.skip_row(row):
                continue

            # base selling amount from invoice item net amount (or header's base_net_total)
            ...
            if grouped_by_invoice and row.indent == 0.0:
                # aggregate child rows into header
                row.buying_amount = buying_amount

                # NEW: items-only selling total (without expenses)
                row.total_base_amount = flt(base_amount, self.currency_precision)

                # existing: Selling Amount starts as items total
                row.base_amount = flt(base_amount, self.currency_precision)

                # include extra expenses in Selling Amount
                total_extra_expense = (
                    flt(row.transporter_amount)
                    + flt(row.misc_amount)
                    + flt(row.labour_amount)
                )
                row.base_amount = flt(row.base_amount + total_extra_expense, self.currency_precision)

                # reset for next invoice
                buying_amount = 0
                base_amount = 0

            # get buying rate / base rate
            if flt(row.qty):
            ...
            # add to grouped
            group_key = row.get(scrub(self.filters.get("group_by")))
            self.grouped.setdefault(group_key, []).append(row)
    ...
    def calculate_buying_amount_from_sle(self, row, my_sle, parenttype, parent, item_row, item_code):
        for i, sle in enumerate(my_sle):
            # find the stock valuation rate from stock ledger entry
            if (
    ...
    def load_invoice_items(self):
        ...
        if self.filters.get("warehouse"):
            ...
            if warehouse_details:
                conditions += (
                    f" and `tabSales Invoice Item`.warehouse in (select name from `tabWarehouse` wh where wh.lft >= "
                    f"{warehouse_details.lft} and wh.rgt <= {warehouse_details.rgt} and warehouse = wh.name)"
                )
    ...
    def group_items_by_invoice(self):
        ...
        for row in self.si_list:
            ...
            if row.parent not in grouped:
                grouped[row.parent] = [self.get_invoice_row(row)]
            ...
            if bundled_items := product_bundles.get(row.parent, {}).get(row.item_code):
                for x in bundled_items:
                    ...
                    grouped[row.parent].append(bundle_item)
        ...
        return frappe._dict(
            {
                ...
                # NEW: safe default so formatter doesn't break
                "total_base_amount": 0,
            }
        )
