...
def get_conditions(filters):
	...
	# ✅ REMOVED status filter (because removed from JS)

	if filters.get("warehouse"):
		conditions += " and soi.warehouse = %(warehouse)s"

	# ✅ Only show records where pending_qty > 0
	conditions += " and (soi.qty - IFNULL(soi.delivered_qty, 0)) > 0"
	...
	data = frappe.db.sql(
		...
		SELECT
			...
			CASE
				...
				ELSE GREATEST(DATEDIFF(CURRENT_DATE, soi.delivery_date), 0)
			END as delay,
		...
		WHERE
			so.status not in ('Stopped', 'On Hold')
			and so.docstatus = 1
			{conditions}
		GROUP BY
			soi.name
		ORDER BY
			so.transaction_date ASC, soi.item_code ASC
	""",
		filters,
		as_dict=1,
	)
...
def prepare_data(data, so_elapsed_time, filters):
	...
	for row in data:
		...
		row["time_taken_to_deliver"] = (
			...
			if row.get("status") in ("To Bill", "Completed")
			else 0
		)
	...
	if filters.get("group_by_so"):
		data = [sales_order_map[so] for so in sales_order_map]
...
def get_columns(filters):
	...
	if not filters.get("group_by_so"):
		...
		columns.append({"label": _("Description"), "fieldname": "description", "fieldtype": "Small Text", "width": 100})

	columns.extend(
		[
			{"label": _("Qty"), "fieldname": "qty", "fieldtype": "Float", "width": 120, "convertible": "qty"},
			{"label": _("Delivered Qty"), "fieldname": "delivered_qty", "fieldtype": "Float", "width": 120, "convertible": "qty"},
			{"label": _("Qty to Deliver"), "fieldname": "pending_qty", "fieldtype": "Float", "width": 120, "convertible": "qty"},
			...
			{"label": _("Time Taken to Deliver"), "fieldname": "time_taken_to_deliver", "fieldtype": "Duration", "width": 100},
		]
	)
	...
	columns.append({"label": _("Company"), "fieldname": "company", "fieldtype": "Link", "options": "Company", "width": 100})
