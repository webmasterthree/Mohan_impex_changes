...
#         if late_checkins == 3:
#             frappe.msgprint(_("Exactly 3 late check-ins detected. Checking leave availability..."))
...
#                 # Reset late check-in count after leave deduction
#                 reset_late_checkins(doc.employee)
...
# def reset_late_checkins(employee):
#     """Reset late check-in count after leave deduction."""
#     frappe.db.sql("""DELETE FROM `tabEmployee Checkin` WHERE employee = %s AND log_type = 'IN'""", (employee,))
#     frappe.db.commit()
#     frappe.logger().info(f"Late check-ins reset for Employee {employee} after leave deduction.")
